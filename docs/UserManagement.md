...
## 用户 User
我将系统中全部的用户都存储在一张users数据表上，每一行表示一个独一无二的用户，用户的id通过自增主键表示。为防止用户的自增主键ID对外暴露系统的用户量与用户注册时间等信息，我为每个用户另外增设了一个UID用于对外显示该用户的唯一标识。UID基于uesrId生成，算法如下：
UID = p * userId %q
p和q都是系统内定义的素数，p > q > userId，p和q可通过配置文件进行设置，但必须对外保密。
